...
export class CustomerService {
  ...
  getCustomerOrdersBeforeDate(customerKey: string, beforeDate: Date): Observable<OrderWithCustomerData[]> {
    const params = new HttpParams().set('before', beforeDate.toISOString().split('T')[0]);
    return this.http.get<OrderWithCustomerData[]>(`${this.apiUrl}/${customerKey}/order`, { params });
  }
